...
import { funcoesCrud } from '@/data/hooks/useCrudGenerico'
...
export default function FuncoesPage() {
  ...
  const { data: response, isLoading, refetch } = funcoesCrud.useListar({ search: search || undefined })
  const criar = funcoesCrud.useCriar()
  const atualizar = funcoesCrud.useAtualizar()
  const excluir = funcoesCrud.useExcluir()
  ...
  return (
    <div>
      <Text size="xs" c="dimmed" mb={4}>Início / Configurador / Funções</Text>
      <Text size="xl" fw={600} mb="lg">Funções</Text>
      ...
      <Modal opened={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Editar Função' : 'Nova Função'} centered closeOnClickOutside={false}>
