...
import { ambientesArmazenagemCrud } from '@/data/hooks/useCrudGenerico'

const schema = z.object({ descricao: z.string().min(1, 'Descrição é obrigatória'), temperatura: z.string().optional() })
type FormValues = z.infer<typeof schema>

const TEMPERATURAS = [{ value: 'SECO', label: 'Seco' }, { value: 'REFRIGERADO', label: 'Refrigerado' }, { value: 'CONGELADO', label: 'Congelado' }, { value: 'CLIMATIZADO', label: 'Climatizado' }]
const tempColor: Record<string, string> = { SECO: 'orange', REFRIGERADO: 'blue', CONGELADO: 'indigo', CLIMATIZADO: 'primary' }

export default function AmbienteArmazenagemPage() {
  const [modalOpen, setModalOpen] = useState(false)
  const [editItem, setEditItem] = useState<any>(null)
  const [search, setSearch] = useState('')

  const { data: response, isLoading, refetch } = ambientesArmazenagemCrud.useListar({ search: search || undefined })
  const criar = ambientesArmazenagemCrud.useCriar()
  const atualizar = ambientesArmazenagemCrud.useAtualizar()
  const excluir = ambientesArmazenagemCrud.useExcluir()
  const { control, handleSubmit, reset, formState: { errors } } = useForm<FormValues>({ resolver: zodResolver(schema) })

  function handleNew() { setEditItem(null); reset({ descricao: '', temperatura: '' }); setModalOpen(true) }
  function handleEdit(item: any) { setEditItem(item); reset({ descricao: item.descricao, temperatura: item.temperatura || '' }); setModalOpen(true) }
  ...
  return (
    <div>
      <Text size="xs" c="dimmed" mb={4}>Início / Configurador / Ambiente Armazenagem</Text>
      <Text size="xl" fw={600} mb="lg">Ambiente de Armazenagem</Text>
      <Card pos="relative">
        ...
        <Table striped highlightOnHover>
          <Table.Thead><Table.Tr><Table.Th>Código</Table.Th><Table.Th>Descrição</Table.Th><Table.Th>Temperatura</Table.Th><Table.Th>Status</Table.Th><Table.Th className="w-24">Ações</Table.Th></Table.Tr></Table.Thead>
          <Table.Tbody>{items.map((item: any) => (
            <Table.Tr key={item.id}><Table.Td>{item.codigo}</Table.Td><Table.Td>{item.descricao}</Table.Td>
              <Table.Td>{item.temperatura && <Badge color={tempColor[item.temperatura] || 'gray'} variant="light">{item.temperatura}</Badge>}</Table.Td>
      ...
      <Modal opened={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Editar Ambiente' : 'Novo Ambiente'} centered closeOnClickOutside={false}>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-col gap-4">
            ...
            <Controller name="temperatura" control={control} render={({ field }) => (<Select label="Temperatura" data={TEMPERATURAS} clearable {...field} />)} />
          </div>
