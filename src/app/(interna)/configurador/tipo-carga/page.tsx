...
import { tiposCargaCrud } from '@/data/hooks/useCrudGenerico'

const schema = z.object({ descricao: z.string().min(1, 'Obrigatório') })
type FormValues = z.infer<typeof schema>

export default function TipoCargaPage() {
  const [modalOpen, setModalOpen] = useState(false)
  const [editItem, setEditItem] = useState<any>(null)
  const [search, setSearch] = useState('')
  const { data: response, isLoading, refetch } = tiposCargaCrud.useListar({ search: search || undefined })
  const criar = tiposCargaCrud.useCriar()
  const atualizar = tiposCargaCrud.useAtualizar()
  const excluir = tiposCargaCrud.useExcluir()
  ...
  return (
    <div>
      <Text size="xs" c="dimmed" mb={4}>Início / Configurador / Tipo Carga</Text>
      <Text size="xl" fw={600} mb="lg">Tipo de Carga</Text>
