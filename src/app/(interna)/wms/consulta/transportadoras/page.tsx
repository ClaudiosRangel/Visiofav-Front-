...
export default function ConsultaTransportadorasPage() {
  useModuloGuard('WMS')
  useEffect(() => { document.title = 'VisioFab - WMS - Consulta Transportadoras' }, [])
  const [search, setSearch] = useState('')
  const [page, setPage] = useState(1)
  const limit = 20

  const { data: response, isLoading } = useQuery<any>({
    queryKey: ['transportadoras', { busca: search || undefined, page, limit }],
    queryFn: async () => {
      const params: Record<string, unknown> = { page, limit }
      if (search) params.busca = search
      const { data } = await api.get('/transportadoras', { params })
      return data
    },
  })
  ...
  return (
    <div>
      <Text size="xs" c="dimmed" mb={4}>WMS / Consulta / Transportadoras</Text>
      <Text size="xl" fw={600} mb="lg">Transportadoras (Consulta)</Text>

      <Alert icon={<IconLock size={16} />} color="blue" variant="light" mb="md">
        Visualização somente leitura. Para cadastrar ou editar transportadoras, acesse o módulo de Compras.
      </Alert>
      ...
      <Card pos="relative">
        ...
        <Table striped highlightOnHover>
          <Table.Thead>
            <Table.Tr>
              <Table.Th>Razão Social</Table.Th><Table.Th>Nome Fantasia</Table.Th><Table.Th>CNPJ</Table.Th>
          ...
          <Table.Tbody>
            {items.map((item: any) => (
              <Table.Tr key={item.id}>
                ...
                <Table.Td className="font-mono text-sm">{item.cnpj || '—'}</Table.Td>
            ...
            {!isLoading && items.length === 0 && <Table.Tr><Table.Td colSpan={6} className="text-center py-8 text-zinc-500">Nenhuma transportadora cadastrada</Table.Td></Table.Tr>}
          </Table.Tbody>
        </Table>
