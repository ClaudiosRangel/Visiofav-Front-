...
export default function ClienteModal({ opened, onClose, editData }: Props) {
  ...
  const atualizar = useMutation({
    mutationFn: async ({ id, ...body }: any) => { const { data } = await api.put(`/clientes/${id}`, body); return data },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['clientes'] }),
  })
  ...
  async function onSubmit(data: FormValues) {
    try {
      ...
      notifications.show({ title: 'Sucesso', message: isEditing ? 'Cliente atualizado' : 'Cliente criado', color: 'green' })
      onClose()
    } catch (err: any) {
