...
/**
 * Retorna a aba de um centro com base no cÃ³digo do Tipo de Processo
 * cadastrado (cadastro dinÃ¢mico em PCP â†’ Cadastros â†’ Tipo de Processo,
 * substitui o antigo enum fixo tipoMaquina). Cada Tipo de Processo ativo
 * gera sua prÃ³pria aba â€” o cÃ³digo do tipo (ex: "CORTADEIRA") Ã© usado como
 * chave da aba. Centro sem tipo de processo (nÃ£o deveria ocorrer, jÃ¡ que o
 * campo Ã© obrigatÃ³rio) cai em 'outros' como salvaguarda.
 */
...
/**
 * Regra de negÃ³cio confirmada pelo usuÃ¡rio: sÃ³ as mÃ¡quinas de Colagem/
 * Coladeira contam a produÃ§Ã£o em "embalagem" (unidade fechada, unitÃ¡rio) â€”
 * todas as outras (ImpressÃ£o, Cortadeira, demais Acabamentos) contam em
...
function getRowBackground(etapa: any, usaCoresStatus: boolean = true): string | undefined {
  // OP avulsa tem prioridade visual sobre as cores de status â€” precisa ser
  // reconhecida de imediato, independente do estÃ¡gio da produÃ§Ã£o, e NÃƒO Ã©
  // afetada pela flag "usaCoresStatusProgramacao" (ConfiguraÃ§Ã£o PCP).
  ...
  // sÃ³lidos "-0": os swatches sÃ£o fixos independente do tema, o que deixava
  ...
  // Atrasada: entrega < hoje e nÃ£o concluÃ­da
    ...
    <Table.Tr ref={setNodeRef} style={style} {...attributes} data-etapa-id={etapa.id}>
      ...
      <Table.Td style={{ width: 30, padding: '0 4px' }} onPointerDown={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}>
        <div onClick={() => onToggleSelect?.()} style={{ cursor: 'pointer', width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #adb5bd', borderRadius: 3, background: selected ? '#228be6' : 'transparent' }}>
          {selected && <span style={{ color: '#fff', fontSize: 12, fontWeight: 'bold' }}>âœ“</span>}
        </div>
      </Table.Td>
      {children}
    </Table.Tr>
...
export default function ProgramacaoPage() {
  ...
  /**
   * Verifica se o usuÃ¡rio tem permissÃ£o para executar uma aÃ§Ã£o no painel,
   * considerando as permissÃµes granulares por tipo de processo.
  ...
  function podeExecutar(acao: string, tipoProcessoId?: string | null, permissoes?: any): boolean {
    ...
    // Se hÃ¡ configuraÃ§Ã£o especÃ­fica para este tipo de processo, usa ela
  ...
  // Aba ativa no Modelo 1 (Grid) â€” chave Ã© o cÃ³digo do Tipo de Processo em
  // minÃºsculas (ex: 'cortadeira'), gerado dinamicamente a partir do cadastro
  ...
  // Layout da tela: "grid" (tabelas por centro, padrÃ£o atual) ou "detalhado"
  // (lista mestre + painel de detalhe Ãºnico, evita repetir a mesma OP em
  // vÃ¡rias linhas/abas). Persistido para lembrar a preferÃªncia do usuÃ¡rio.
  ...
  // Configurador de colunas de impressÃ£o por tipo de processo.
  // Cada tipo de processo pode ter um subconjunto de colunas selecionadas
  // para aparecer na impressÃ£o. Salvo em localStorage.
  const COLUNAS_DISPONIVEIS = [
    ...
    { id: 'material', label: 'Material/CartÃ£o' },
  ...
  function abrirConfigColunas() {
    // Inicializa o estado de ediÃ§Ã£o com as colunas atuais de cada tipo de processo
  ...
  function salvarConfigColunas() {
    ...
    notifications.show({ title: 'Colunas salvas', message: 'ConfiguraÃ§Ã£o de impressÃ£o atualizada', color: 'green' })
  }
  ...
  // Modais
  // `finalizando`: quando true, o modal foi aberto pelo botÃ£o "Finalizar" â€”
  // ao salvar, registra o apontamento E conclui a etapa em seguida (a etapa
  // sai da fila do grupo), em vez de sÃ³ registrar produÃ§Ã£o parcial.
  // tipoProcessoCodigo do centro da etapa â€” define a unidade de contagem
  // exibida no modal: mÃ¡quinas de Colagem/Coladeira contam em "embalagem"
  // (unitÃ¡rio), as demais em "folhas/tiragem" (regra de negÃ³cio confirmada
  // pelo usuÃ¡rio).
  ...
  // Foto anexada pelo operador como evidÃªncia da contagem produzida (opcional)
  ...
  // Trava de duplo clique/duplo submit no botÃ£o "Adicionar Ã  Fila"
  ...
  // (herdar de uma OP jÃ¡ cadastrada vs escolher produto/cliente livremente)
  ...
  // clienteNome guarda o texto digitado/selecionado; clienteId sÃ³ Ã© preenchido
  ...
  // ConfiguraÃ§Ã£o de empresa (ConfiguraÃ§Ã£o PCP) que habilita/desabilita as
  // cores de status na fila, nos dois layouts (Grid e Detalhado). A cor de
  // OP Avulsa nunca Ã© afetada por essa flag â€” ver getRowBackground. Default
  // Default false â€” se a empresa habilitou cores, a API corrige ao carregar.
  // Antes era true (mostrava cores brevemente mesmo desabilitado). Agora
  // respeita: se a API nÃ£o responder, fica sem cores (seguro).
  const [usaCoresStatus, setUsaCoresStatus] = useState(false)
  // Tipos de Processo ATIVOS cadastrados (PCP â†’ Cadastros â†’ Tipo de
  // Processo), ordenados por posiÃ§Ã£o â€” cada um gera uma aba no painel,
  // substituindo a lista fixa (Cortadeira/ImpressÃ£o/Acabamento/Outros) que
  // existia antes no cÃ³digo.
  const [tiposProcesso, setTiposProcesso] = useState<any[]>([])

  // Etapas concluÃ­das (visualizaÃ§Ã£o por processo com opÃ§Ã£o de retornar)
  ...
  // SeleÃ§Ã£o mÃºltipla de etapas para aÃ§Ãµes em lote
  ...
  async function acaoLoteIniciar() {
    ...
    notifications.show({ title: 'Lote concluÃ­do', message: `${sucesso} etapa(s) iniciada(s)`, color: 'green' })
    limparSelecao()
  ...
  async function acaoLoteFinalizar() {
    ...
    notifications.show({ title: 'Lote concluÃ­do', message: `${sucesso} etapa(s) finalizada(s)`, color: 'green' })
    limparSelecao()
    setAcaoLoteLoading(false)
    try { const { data } = await api.get('/pcp/programacao/painel'); setPainel(data) } catch {}
  }
  ...
  async function acaoLoteMover() {
    ...
    notifications.show({ title: 'Lote concluÃ­do', message: `${sucesso} etapa(s) movida(s)`, color: 'green' })
    limparSelecao()
  ...
  async function acaoLoteReextrair() {
    ...
    setAcaoLoteLoading(true)
    // Coletar opIds Ãºnicos das etapas selecionadas
    ...
    notifications.show({ title: 'Lote concluÃ­do', message: `${sucesso} OP(s) re-extraÃ­da(s)`, color: 'green' })
    limparSelecao()
    setAcaoLoteLoading(false)
    try { const { data } = await api.get('/pcp/programacao/painel'); setPainel(data) } catch {}
  }
  ...
  async function carregarConcluidas(tipoProcessoId?: string) {
    ...
    try {
      ...
      notifications.show({ title: 'Erro', message: 'Falha ao carregar concluÃ­das', color: 'red' })
    } finally {
      setLoadingConcluidas(false)
    }
  }

  // Recarrega concluÃ­das ao trocar de aba, se o painel de concluÃ­das estiver aberto
  useEffect(() => {
  ...
  async function retornarEtapa() {
    ...
    try {
      ...
      notifications.show({ title: 'Sucesso', message: `OS ${modalRetornar.opNumero} retornada Ã  fila`, color: 'green' })
      const etapaRetornada = modalRetornar.etapaId
      setModalRetornar(null)
      setFormRetornar({ emailAdmin: '', senhaAdmin: '' })
      // Remove da lista de concluÃ­das
      ...
      // para que a etapa retornada apareÃ§a com todos os dados corretos
      try {
        const { data } = await api.get('/pcp/programacao/painel')
        setPainel(data)
      } catch { /* silencioso â€” dados aparecem na prÃ³xima recarga manual */ }
    } catch (err: any) {
  ...
  // PermissÃµes PCP do usuÃ¡rio logado (carregadas uma vez)
  ...
  // que a lista carrega â€” sÃ³ na primeira carga (activeTab ainda vazio), para
  // nÃ£o sobrescrever a escolha do usuÃ¡rio em cargas subsequentes (carregar()
  // Ã© chamado novamente apÃ³s aÃ§Ãµes como criar/mover/reordenar).
  useEffect(() => {
  ...
  function handleCentroDragEnd(event: DragEndEvent) {
    ...
    // Verificar permissÃ£o de reordenar grupos
    if (!minhasPermissoes.isAdmin && !isAdmin && !minhasPermissoes.podeReordenarGrupos) {
      notifications.show({ title: 'Sem permissÃ£o', message: 'VocÃª nÃ£o tem permissÃ£o para reordenar grupos', color: 'red' })
      return
    }
  ...
  // Reordena a fila de um centro via API â€” usada pelo drag-and-drop da lista
  // mestre do layout Detalhado (VisaoDetalhadaProgramacao). O estado otimista
  // e o rollback em caso de erro sÃ£o tratados dentro do prÃ³prio componente;
  // aqui sÃ³ persiste no backend e recarrega o painel ao final para manter
  ...
  // Cicla a prioridade da OP (BAIXAâ†’NORMALâ†’ALTAâ†’URGENTEâ†’BAIXA) â€” mesma lÃ³gica
  // jÃ¡ usada inline no clique da cÃ©lula "Prio." do modelo ImpressÃ£o/Acabamento
  // do Grid, extraÃ­da aqui para ser reaproveitada pelo layout Detalhado.
  function alterarPrioridade(opId: string, prioridadeAtual: string) {
    ...
    api.patch(`/ordens-producao/${opId}`, { prioridade: nova }).then(() => {
      // AtualizaÃ§Ã£o otimista: atualiza a prioridade de todas as etapas
  ...
  async function handleDragEnd(centroId: string, event: DragEndEvent) {
    ...
    // Verificar permissÃ£o de reordenar fila
    if (!minhasPermissoes.isAdmin && !isAdmin && !minhasPermissoes.podeReordenarFila) {
      notifications.show({ title: 'Sem permissÃ£o', message: 'VocÃª nÃ£o tem permissÃ£o para reordenar a fila', color: 'red' })
      return
    }
    ...
    // Persist â€” active.id Ã© a etapa que o usuÃ¡rio efetivamente arrastou;
    // sÃ³ ela Ã© marcada como ordemManual=true no backend (posiÃ§Ã£o fixa,
    // sobrepÃµe os critÃ©rios automÃ¡ticos de nÂº OP â†’ data de entrega).
    try {
  ...
  // Atualiza campos de UMA etapa especÃ­fica dentro de `painel.centros`, sem
  // recarregar o painel inteiro do backend â€” usado por aÃ§Ãµes rÃ¡pidas
  // (iniciar/pausar) que nÃ£o devem causar loading global, recolapsar grupos
  // que o usuÃ¡rio abriu/fechou manualmente, nem reordenar a fila (a busca
  // completa do painel pode vir em ordem levemente diferente a cada chamada).
  // `carregar()` continua sendo usado sÃ³ onde a aÃ§Ã£o de fato precisa de dados
  // recalculados no servidor (nova etapa, conclusÃ£o que afeta a fila, etc.).
    ...
    setPainel((prev: any) => {
      ...
      return {
        ...
        centros: prev.centros.map((c: any) => {
          ...
          if (etapas === c.etapas) return c // etapa nÃ£o pertence a este centro â€” nada mudou
          // Recalcula o resumo (badges "X em andamento"/"X pausadas"/"X pendentes"
          // no cabeÃ§alho do grupo) para refletir a mudanÃ§a de status, com a
  ...
  // Abre o modal de apontamento em modo "finalizar": o usuÃ¡rio registra a
  // quantidade produzida final e, ao confirmar, a etapa Ã© apontada e
  // concluÃ­da na sequÃªncia (continua saindo da fila do grupo, como antes).
  function abrirFinalizarEtapa(etapa: any, tipoProcessoCodigo?: string | null) {
    // Finaliza direto sem pedir apontamento â€” a quantidade produzida Ã©
    // registrada como o saldo restante automaticamente (produÃ§Ã£o completa).
  ...
  async function concluirEtapaDireta(etapaId: string, opNumero?: number) {
    try {
      await api.patch(`/pcp/etapas/${etapaId}/concluir`, {})
      notifications.show({ title: 'Etapa finalizada', message: `OS ${opNumero || ''} concluÃ­da`, color: 'green' })
      // AtualizaÃ§Ã£o otimista: remove a etapa concluÃ­da da fila
      setPainel((prev: any) => {
        if (!prev) return prev
        const centros = prev.centros.map((c: any) => {
  ...
  async function enviarApontamento() {
    ...
    try {
      ...
      if (modalApontar.finalizando) {
        // AtualizaÃ§Ã£o otimista: remove a etapa concluÃ­da da fila sem
        // recarregar o painel inteiro (evita loading/spinner e reordenaÃ§Ã£o).
        ...
        // Apontamento parcial (nÃ£o finalizou): atualiza quantidade local
  ...
  async function salvarObservacao(etapaId: string, valor: string) {
    try {
      ...
      notifications.show({ title: 'Erro', message: 'Falha ao salvar observaÃ§Ã£o', color: 'red' })
    }
    setEditingObs(null)
  }
  ...
  async function postergarEntrega(opId: string, novaData: string) {
    try {
      // Adiciona horÃ¡rio meio-dia para evitar problema de timezone
      ...
      // AtualizaÃ§Ã£o otimista: atualiza a data de entrega de todas as etapas
      // dessa OP no painel local, sem recarregar tudo (preserva ordem da fila)
      setPainel((prev: any) => {
        if (!prev) return prev
        return {
  ...
  async function excluirEtapa(etapaId: string, isDesmembramento: boolean) {
    const msg = isDesmembramento
      ? 'Reverter desmembramento? A quantidade serÃ¡ somada na etapa irmÃ£.'
      : 'Excluir este lanÃ§amento manual da fila?'
    ...
    try {
      if (isDesmembramento) {
        ...
        notifications.show({ title: 'LanÃ§amento excluÃ­do', message: 'Etapa removida da fila', color: 'green' })
      }
      carregar()
    } catch (err: any) {
  ...
  async function reverterDesmembramento(etapaId: string) {
    if (!confirm('Reverter desmembramento? A quantidade serÃ¡ somada de volta na etapa irmÃ£.')) return
    try {
      await api.delete(`/pcp/etapas/${etapaId}/reverter-desmembramento`)
      notifications.show({ title: 'Desmembramento revertido', message: 'Quantidade devolvida Ã  etapa restante', color: 'green' })
      carregar()
    } catch (err: any) {
  ...
  async function moverEtapaParaGrupo(etapaId: string, novoCentroId: string) {
    try {
      ...
      setModalMover(null)
      // AtualizaÃ§Ã£o otimista: move a etapa do centro antigo para o novo sem
      // recarregar o painel inteiro (evita loading/spinner e reordenaÃ§Ã£o).
      setPainel((prev: any) => {
        if (!prev) return prev
  ...
  async function renomearGrupo(centroId: string, novaDescricao: string) {
    try {
      // Busca dados atuais do centro para enviar todos os campos obrigatÃ³rios
  ...
  async function verPdfOp(opId: string) {
    try {
      ...
      notifications.show({ title: 'PDF nÃ£o disponÃ­vel', message: 'PDF nÃ£o encontrado para esta OP', color: 'orange' })
    }
  }
  ...
  async function reextrairPdf(opId: string, opNumero: string | number) {
    try {
      ...
      notifications.show({
        ...
        message: partes.length > 0 ? partes.join(' | ') : 'Nenhuma informaÃ§Ã£o nova encontrada no PDF',
        color: partes.length > 0 ? 'green' : 'orange',
      })
      if (avisos?.length > 0) {
        notifications.show({ title: 'AtenÃ§Ã£o', message: avisos.join(' '), color: 'yellow' })
      }
      // NÃ£o faz carregar() â€” mantÃ©m a ordem das OPs na tela (mesmo
      // comportamento do iniciar). Os dados extraÃ­dos sÃ£o metadados
      // (tipo, matriz, formato, materiais) que nÃ£o afetam a fila.
    } catch (err: any) {
  ...
  async function liberarProducao(opId: string) {
    try {
      // Marca material como recebido â€” remove "encomendado" das observaÃ§Ãµes da OP
      ...
      notifications.show({ title: 'Material recebido', message: 'CartÃ£o recebido â€” OS liberada do aguardo', color: 'green' })
      // AtualizaÃ§Ã£o otimista: remove do quadro "Aguardando CartÃ£o" sem
      // recarregar o painel inteiro (evita loading/spinner e reordenaÃ§Ã£o).
      setPainel((prev: any) => {
        if (!prev) return prev
        return {
      ...
      // Recarrega em background (sem loading) para que a OP apareÃ§a nos
  ...
  // Feature 4: Localizar OS â€” when busca is a pure number and user presses Enter
  function localizarOS() {
    ...
    notifications.show({ title: 'OS nÃ£o encontrada', message: `Nenhuma OS #${numero} encontrada no painel`, color: 'orange' })
  }

  // Feature 5a: Criar novo grupo (centro de produÃ§Ã£o)
  async function criarNovoGrupo() {
    ...
    // Verificar se jÃ¡ existe grupo com mesma descriÃ§Ã£o
    const grupoExistente = painel?.centros?.find((c: any) =>
      c.centro.descricao.toLowerCase() === descricao.toLowerCase()
    )
    if (grupoExistente) {
      notifications.show({ title: 'Grupo jÃ¡ existe', message: `JÃ¡ existe um grupo "${descricao}"`, color: 'orange' })
      return
    }
    try {
  ...
  async function buscarOpParaAdicionar() {
    ...
    try {
      ...
      if (ops.length > 0) {
        ...
        notifications.show({ title: 'OP nÃ£o encontrada', message: `Nenhuma OP #${formAdicionarOS.opNumero}`, color: 'orange' })
      }
    } catch (err: any) {
  ...
  async function confirmarAdicionarOS() {
    // Guarda contra duplo clique/duplo submit â€” sem isso, cliques rÃ¡pidos
    // repetidos no botÃ£o (ou double-submit de rede) criavam duas etapas
    // idÃªnticas para a mesma OP no mesmo centro (bug real encontrado na
    // OP 2898, grupo "ServiÃ§os Manuais - ProduÃ§Ã£o").
    ...
    try {
      ...
      notifications.show({ title: 'OS adicionada', message: `OP #${formAdicionarOS.opNumero} adicionada Ã  fila`, color: 'green' })
  ...
  // OP Avulsa â€” carrega produtos/clientes disponÃ­veis para o modo "livre"
  function carregarProdutosEClientes() {
    ...
    if (clientesDisponiveis.length === 0) {
      // Combina cadastro formal + nomes extraÃ­dos de OPs (a maioria nÃ£o tem
      // cliente cadastrado, sÃ³ o nome em texto vindo do PDF importado)
  ...
  // OP Avulsa â€” modo "herdar": busca a OP de origem pelo nÃºmero, para copiar produto/cliente
  async function buscarOpOrigemAvulsa() {
    ...
    try {
      ...
      if (ops.length > 0) {
        ...
        notifications.show({ title: 'OP nÃ£o encontrada', message: `Nenhuma OP #${formAvulsaOrigem.opNumero}`, color: 'orange' })
      }
    } catch (err: any) {
      notifications.show({ title: 'Erro', message: 'Falha ao buscar OP', color: 'red' })
  ...
  // OP Avulsa â€” cria a OP (AV-1, AV-2...) jÃ¡ na fila do centro, herdando de
  ...
  async function confirmarAdicionarAvulsa() {
    ...
    if (modoAvulsa === 'herdar' && !opOrigemEncontrada) {
      notifications.show({ title: 'Busque a OP de origem', message: 'Informe o nÃºmero e clique em Buscar', color: 'orange' })
      return
    }
    ...
    // Nome livre do cliente: no modo "herdar", a OP de origem pode nÃ£o ter
    // clienteId real, sÃ³ o nome extraÃ­do do PDF (clienteNome computado pelo
    ...
    // Nome livre do produto: sÃ³ existe no modo "livre" quando o usuÃ¡rio
    // digitou um texto que nÃ£o corresponde a nenhum produto cadastrado.
    ...
    try {
      ...
      notifications.show({ title: 'OP avulsa criada', message: `${res.data.referenciaAvulsa} adicionada Ã  fila`, color: 'green' })
  ...
  // OP Avulsa â€” exclui a qualquer momento (sem as restriÃ§Ãµes de OP normal)
  async function excluirOpAvulsa(opId: string, referencia: string) {
    if (!confirm(`Excluir a OP avulsa ${referencia}? Esta aÃ§Ã£o nÃ£o pode ser desfeita.`)) return
    try {
      await api.delete(`/pcp/ordens-avulsas/${opId}`)
      notifications.show({ title: 'OP avulsa excluÃ­da', message: referencia, color: 'green' })
      carregar()
    } catch (err: any) {
      notifications.show({ title: 'Erro', message: err?.response?.data?.message || 'Falha ao excluir', color: 'red' })
    }
  ...
  function imprimirRelatorio() {
    ...
    // Mapa de coluna â†’ { header, align, render }
    const colDefs: Record<string, { header: string; align?: string; render: (e: any) => string }> = {
      ...
      cliente: { header: 'Cliente', render: (e) => `<td>${e.clienteNome || 'â€”'}</td>` },
      produto: { header: 'Produto', render: (e) => `<td>${e.produtoNome || 'â€”'}</td>` },
      tipoOp: { header: 'Tipo OP', render: (e) => `<td>${e.tipoOp || 'â€”'}</td>` },
      quantidade: { header: 'Qtd', align: 'right', render: (e) => `<td class="right">${e.quantidade?.toLocaleString('pt-BR') || 'â€”'}</td>` },
      tiragem: { header: 'Tiragem', align: 'right', render: (e) => `<td class="right">${e.tiragem ? e.tiragem.toLocaleString('pt-BR') : 'â€”'}</td>` },
      entrega: { header: 'Entrega', render: (e) => `<td>${e.dataEntrega ? new Date(e.dataEntrega).toLocaleDateString('pt-BR') : 'â€”'}</td>` },
      material: { header: 'Material/CartÃ£o', render: (e) => `<td>${e.materialPrincipal || 'â€”'}</td>` },
      gramatura: { header: 'Gramatura', render: (e) => `<td>${e.gramatura || 'â€”'}</td>` },
      formato: { header: 'Formato', render: (e) => `<td>${e.formato || 'â€”'}</td>` },
      matriz: { header: 'Matriz', render: (e) => `<td>${e.matriz || 'â€”'}</td>` },
      cores: { header: 'Cores', align: 'center', render: (e) => `<td class="center">${e.qtdCores || 'â€”'}</td>` },
      pantone01: { header: 'Pantone 1', render: (e) => `<td>${e.pantone01 || 'â€”'}</td>` },
      pantone02: { header: 'Pantone 2', render: (e) => `<td>${e.pantone02 || 'â€”'}</td>` },
      pantone03: { header: 'Pantone 3', render: (e) => `<td>${e.pantone03 || 'â€”'}</td>` },
      kg: { header: 'KG', align: 'right', render: (e) => `<td class="right">${e.pesoKg ? e.pesoKg.toLocaleString('pt-BR') : 'â€”'}</td>` },
      prioridade: { header: 'Prioridade', render: (e) => `<td>${e.prioridade || 'â€”'}</td>` },
      observacao: { header: 'Acompanhamento', render: (e) => `<td>${e.observacaoOperador || 'â€”'}</td>` },
    }

    let html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>ProgramaÃ§Ã£o de ProduÃ§Ã£o</title>
    <style>
  ...
  // categorizaÃ§Ã£o por aba passa a ser feita dentro do prÃ³prio painel de
  // detalhe, por OP selecionada) â€” mantido aqui sÃ³ para esse caso.
  ...
  const centrosFiltrados = (layoutView === 'detalhado'
    ...
    // SÃ³ ocultar grupos vazios quando hÃ¡ filtro de busca/status/prioridade/data ativo
  ...
  return (
    <Stack gap="md">
      <Title order={3}>Painel Operacional â€” ProgramaÃ§Ã£o por Centro</Title>
      <Text size="sm" c="dimmed">Controle em tempo real: inicie, aponte produÃ§Ã£o, registre paradas e conclua etapas.</Text>

      {/* Legenda de cores â€” mesmo cÃ³digo de cores usado em getRowBackground,
          vÃ¡lido para os dois layouts (Grid e Detalhado). As cores de status
          podem ser desabilitadas em ConfiguraÃ§Ã£o PCP (usaCoresStatus) â€” a de
          OP Avulsa Ã© sempre exibida, independente dessa flag. */}
      <Group gap="md" wrap="wrap">
        {usaCoresStatus && (
          <>
            ...
            <Group gap={6} wrap="nowrap"><Box style={{ width: 10, height: 10, borderRadius: 2, background: 'var(--mantine-color-green-light)', border: '1px solid var(--mantine-color-green-6)' }} /><Text size="xs" c="dimmed">ConcluÃ­da</Text></Group>
      ...
      <Group justify="space-between" align="flex-end">
        {/* No layout Detalhado, o filtro por estÃ¡gio passa a ser feito DENTRO
            do painel de detalhe (abas Cortadeira/ImpressÃ£o/Acabamento por OP
            selecionada) â€” as abas do topo ficam redundantes e sÃ£o ocultadas
            (centrosFiltrados usa layoutView, nÃ£o activeTab, nesse caso). Ao
            voltar para o Grid, as abas voltam a aparecer, mantendo a Ãºltima
            aba selecionada. */}
        {layoutView === 'grid' ? (
          <Tabs value={activeTab} onChange={(value) => setActiveTab(value || tiposProcesso[0]?.codigo.toLowerCase() || 'outros')} style={{ flex: 1 }}>
            <Tabs.List>
              {/* Abas geradas dinamicamente a partir do cadastro Tipo de
                  Processo (PCP â†’ Cadastros â†’ Tipo de Processo), na ordem de
                  posiÃ§Ã£o definida lÃ¡ â€” substitui a lista fixa que existia
                  antes no cÃ³digo. */}
        ...
        <ActionIcon size="sm" variant="subtle" onClick={() => abrirConfigColunas()} title="Configurar colunas de impressÃ£o" className="no-print">
          <IconSettings size={14} />
        </ActionIcon>
        <Button size="xs" variant="light" leftSection={<IconPlus size={14} />} onClick={() => {
          // Pre-seleciona o Tipo de Processo correspondente Ã  aba ativa,
          // buscando pelo cÃ³digo (ex: aba 'cortadeira' â†’ tipo com cÃ³digo
          // 'CORTADEIRA'). Se nÃ£o encontrar (aba 'outros' ou tipo inativo),
          // deixa em branco para seleÃ§Ã£o manual.
      ...
      <>
      {/* SeÃ§Ã£o AGUARDANDO CARTÃƒO â€” filtrada por tipoMaquina conforme aba ativa */}
      {mostrarAguardandoCartao && (
        <Card withBorder padding="xs" style={{ borderColor: 'var(--mantine-color-yellow-5)', background: 'var(--mantine-color-yellow-light)' }}>
          <Text fw={700} size="lg" c="orange" mb="xs">AGUARDANDO CARTÃƒO</Text>
            ...
            <Table.Tbody>
              {aguardandoCartaoFiltrado.map((item: any) => (
                <Table.Tr key={item.opNumero}>
                  ...
                  <Table.Td>{item.cliente || 'â€”'}</Table.Td>
                  <Table.Td>{item.produto || 'â€”'}</Table.Td>
                  <Table.Td>{item.quantidade?.toLocaleString('pt-BR')} {item.unidade}</Table.Td>
                  <Table.Td>
                    <Text size="sm">{item.materialPrincipal || 'â€”'}</Text>
                    {item.bobinas?.length > 0 && (
                      <Stack gap={2} mt={4}>
                        {item.bobinas.map((b: any, i: number) => (
                          <Text key={i} size="xs" c={b.status === 'ENCOMENDADO' ? 'red' : 'green'}>
                            {b.status === 'ENCOMENDADO' ? 'â³' : 'âœ“'} {b.descricao} ({b.kg.toLocaleString('pt-BR')} kg)
                          </Text>
                        ))}
                      </Stack>
                    )}
                  </Table.Td>
                  <Table.Td>{item.gramatura || 'â€”'}</Table.Td>
                  <Table.Td>{item.formato || 'â€”'}</Table.Td>
                  <Table.Td>
                    {item.kgEstoque > 0 && <Text size="xs" c="green">âœ“ {item.kgEstoque.toLocaleString('pt-BR')} kg estoque</Text>}
                    {item.kgEncomendado > 0 && <Text size="xs" c="red">â³ {item.kgEncomendado.toLocaleString('pt-BR')} kg encomendado</Text>}
                  </Table.Td>
                  <Table.Td>{item.dataEntrega ? new Date(item.dataEntrega).toLocaleDateString('pt-BR') : 'â€”'}</Table.Td>
                  <Table.Td style={{ minWidth: 150, ...(item.observacaoOperador?.replace('[MATRIZ_OK]', '').trim() ? { background: '#2dd4a8', borderRadius: 2 } : {}) }}>
          ...
          <Text size="xs" c="dimmed" mt="xs">OPs com material (cartÃ£o/bobina) marcado como "encomendado" no PDF da OS</Text>
        </Card>
      )}

      {/* Indicador de salvamento da reordenaÃ§Ã£o de centros */}
      ...
      {/* Barra de aÃ§Ãµes em lote (aparece quando hÃ¡ etapas selecionadas) */}
      ...
      {/* Painel de etapas concluÃ­das (por processo/aba ativa) */}
      {mostrarConcluidas && (
        <Card withBorder padding="md" mb="md">
          <Group justify="space-between" mb="sm">
            <Text fw={600} size="sm">Etapas ConcluÃ­das â€” {tiposProcesso.find((t: any) => t.codigo.toLowerCase() === activeTab)?.descricao || activeTab}</Text>
          ...
          {loadingConcluidas ? (
            ...
            <Text size="sm" c="dimmed" ta="center" py="md">Nenhuma etapa concluÃ­da encontrada para este processo</Text>
          ) : (
            <Table striped highlightOnHover fontSize="xs">
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th>ConcluÃ­da em</Table.Th>
                  <Table.Th>AÃ§Ã£o</Table.Th>
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody>
                {etapasConcluidas.map((ec: any) => (
                  <Table.Tr key={ec.id}>
                    ...
                    <Table.Td>{ec.cliente || 'â€”'}</Table.Td>
                    <Table.Td>{ec.produto || 'â€”'}</Table.Td>
                    ...
                    <Table.Td>{ec.dataFimReal ? new Date(ec.dataFimReal).toLocaleDateString('pt-BR') : 'â€”'}</Table.Td>
                    <Table.Td>
                      <Button
                        size="compact-xs"
                        variant="light"
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleCentroDragEnd}>
        <SortableContext items={centrosFiltrados.map((c: any) => c.centro.id)} strategy={verticalListSortingStrategy}>
          ...
          <Group justify="space-between" py={4} px={8}>
                ...
                <Select
                  ...
                  onChange={(val) => {
                    if (val && val !== centro.centro.id) {
                      // Pega a descriÃ§Ã£o do centro selecionado
            ...
            <Group gap="xs">
              {/* Totalizador de Tiragem do grupo â€” soma da coluna Tiragem de
                  todas as etapas visÃ­veis (jÃ¡ filtradas por busca/status/etc,
                  igual ao total exibido no relatÃ³rio impresso). */}
              {(() => {
                      ...
                      <Table.Tbody>
                        {centro.etapas.map((etapa: any) => (
                          <SortableRow key={etapa.id} etapa={etapa} background={getRowBackground(etapa, usaCoresStatus)} highlighted={highlightedEtapa === etapa.id} selected={selectedEtapas.has(etapa.id)} onToggleSelect={() => toggleSelectEtapa(etapa.id)}>
                            <Table.Td style={{ minWidth: 200 }}>
                              <Group gap={4} wrap="nowrap">
                                <Text size="sm" fw={700} style={{ lineHeight: 1.2 }}>
                                  {etapa.opNumero} â€” {etapa.clienteNome || 'â€”'}
                                </Text>
                            ...
                            <Table.Td>{etapa.tiragem ? etapa.tiragem.toLocaleString('pt-BR') : 'â€”'}</Table.Td>
                            <Table.Td>
                              {etapa.dataEntrega ? (
                            ...
                            <Table.Td><Text size="sm" style={{ wordBreak: 'break-word' }}>{etapa.materialPrincipal || 'â€”'}</Text></Table.Td>
                            <Table.Td>{etapa.gramatura || 'â€”'}</Table.Td>
                            <Table.Td>{etapa.formato || 'â€”'}</Table.Td>
                            <Table.Td>{etapa.pesoKg ? etapa.pesoKg.toLocaleString('pt-BR') : 'â€”'}</Table.Td>
                            ...
                            <Table.Td>
                              <Group gap={2} wrap="nowrap">
                                ...
                                {(etapa.isDesmembramento || etapa.isManual) && etapa.status === 'PENDENTE' && (
                                  <ActionIcon color="red" variant="light" size="sm" onClick={() => excluirEtapa(etapa.id, etapa.isDesmembramento)} title={etapa.isDesmembramento ? 'Reverter desmembramento' : 'Excluir lanÃ§amento manual'}>
                                    <IconX size={14} />
                                  </ActionIcon>
                                )}
                                {etapa.isAvulsa && (
                      ...
                      <Table.Tbody>
                        {centro.etapas.map((etapa: any) => (
                          <SortableRow key={etapa.id} etapa={etapa} background={getRowBackground(etapa, usaCoresStatus)} highlighted={highlightedEtapa === etapa.id} selected={selectedEtapas.has(etapa.id)} onToggleSelect={() => toggleSelectEtapa(etapa.id)}>
                            <Table.Td style={{ minWidth: 200 }}>
                              <Group gap={4} wrap="nowrap">
                                {etapa.observacaoOperador?.includes('[MATRIZ_OK]') && centro.centro.tipoProcesso?.codigo?.toLowerCase().includes('impress') && (
                                  <span style={{ color: '#2f9e44', fontWeight: 'bold', fontSize: 16 }}>âœ“</span>
                                )}
                                <Text size="sm" fw={700} style={{ lineHeight: 1.2 }}>
                                  {etapa.opNumero} â€” {etapa.clienteNome || etapa.observacoes?.match(/\[Cliente\]\s*(.+)/)?.[1] || 'â€”'}
                                </Text>
                                {etapa.isAvulsa && <Badge color="pink" size="xs">AVULSA</Badge>}
                              </Group>
                              <Text size="xs" fw={600} c="dimmed" style={{ lineHeight: 1.2 }}>
                              ...
                              {etapa.materialEncomendado && <Text size="xs" c="red" fw={700}>* Aguardando restante cartÃ£o</Text>}
                            </Table.Td>
                            <Table.Td><Text size="xs" fw={600} c={etapa.tipoOp?.includes('NOVO') ? 'green' : etapa.tipoOp?.includes('REPETI') ? 'blue' : etapa.tipoOp?.includes('ALTERA') ? 'orange' : etapa.tipoOp?.includes('PILOTO') ? 'violet' : 'gray'} style={{ whiteSpace: 'nowrap', fontSize: '10px' }}>{etapa.tipoOp || 'â€”'}</Text></Table.Td>
                            <Table.Td>{etapa.tiragem ? etapa.tiragem.toLocaleString('pt-BR') : 'â€”'}</Table.Td>
                            <Table.Td><Text size="sm" style={{ wordBreak: 'break-word' }}>{etapa.materialPrincipal || 'â€”'}</Text></Table.Td>
                            <Table.Td>{etapa.gramatura || 'â€”'}</Table.Td>
                            <Table.Td>{etapa.formato || 'â€”'}</Table.Td>
                            <Table.Td>{etapa.pesoKg ? `${etapa.pesoKg.toLocaleString('pt-BR')} kg` : 'â€”'}</Table.Td>
                            ...
                            {temApontamento && <Table.Td>{etapa.quantidadePerda > 0 ? <Text c="red" size="sm">{etapa.quantidadePerda}</Text> : 'â€”'}</Table.Td>}
                            ...
                            <Table.Td style={{ minWidth: 100 }}>
                              {etapa.dataEntrega ? (
                                <Group gap={4} wrap="nowrap">
                                  ...
                                  {etapa.vezesPostergada === 0 && <Text size="sm">ðŸŸ¢</Text>}
                                  {etapa.vezesPostergada === 1 && <Text size="sm">ðŸŸ¡</Text>}
                                  {etapa.vezesPostergada >= 2 && <Text size="sm">ðŸ”´</Text>}
                                </Group>
                              ) : 'â€”'}
                            </Table.Td>
                            <Table.Td>
                              <Text size="xs" fw={600} c={PRIORIDADE_COLORS[etapa.prioridade]} style={{ whiteSpace: 'nowrap', fontSize: '10px', cursor: podeExecutar('podeAlterarPrioridade', centro.centro.tipoProcessoId) ? 'pointer' : 'default' }}
                                ...
                                title={podeExecutar('podeAlterarPrioridade', centro.centro.tipoProcessoId) ? 'Clique para alterar prioridade' : 'Sem permissÃ£o para alterar prioridade'}
                              >
                                {etapa.prioridade}
                              </Text>
                            </Table.Td>
                            ...
                            <Table.Td style={etapa.observacaoOperador?.includes('[MATRIZ_OK]') && centro.centro.tipoProcesso?.codigo?.toLowerCase().includes('corte') ? { background: '#2f9e44', borderRadius: 2 } : undefined}><Text size="xs" fw={500} c={etapa.observacaoOperador?.includes('[MATRIZ_OK]') && centro.centro.tipoProcesso?.codigo?.toLowerCase().includes('corte') ? 'white' : undefined}>{etapa.matriz || 'â€”'}</Text></Table.Td>
                            <Table.Td><Text size="xs" fw={600} c="indigo">{etapa.qtdCores || 'â€”'}</Text></Table.Td>
                            <Table.Td><Text size="xs" style={{ whiteSpace: 'nowrap' }}>{etapa.pantone01 || 'â€”'}</Text></Table.Td>
                            <Table.Td><Text size="xs" style={{ whiteSpace: 'nowrap' }}>{etapa.pantone02 || 'â€”'}</Text></Table.Td>
                            <Table.Td><Text size="xs" style={{ whiteSpace: 'nowrap' }}>{etapa.pantone03 || 'â€”'}</Text></Table.Td>
                              ...
                              <Group gap={2} wrap="nowrap">
                                ...
                                {etapa.status === 'EM_ANDAMENTO' && (
                                  <>
                                    ...
                                    <ActionIcon color="blue" variant="light" size="sm" onClick={() => setModalApontar({ etapaId: etapa.id, opNumero: etapa.opNumero, descricao: etapa.descricao, tipoProcessoCodigo: centro.centro.tipoProcesso?.codigo })} title="Apontar ProduÃ§Ã£o">
                                ...
                                {minhasPermissoes.isPreImpressao && (centro.centro.tipoProcesso?.codigo?.toLowerCase().includes('corte') || centro.centro.tipoProcesso?.codigo?.toLowerCase().includes('impress')) && (
                                  <ActionIcon
                                    ...
                                    title={etapa.observacaoOperador?.includes('[MATRIZ_OK]') ? 'Matriz aprovada (clique p/ remover)' : 'Aprovar Matriz (prÃ©-impressÃ£o)'}
                                  >
                                    <IconPalette size={14} />
                                  </ActionIcon>
                                )}
      ...
      {/* Modal: Apontar ProduÃ§Ã£o (tambÃ©m usado ao Finalizar a etapa) */}
      <Modal opened={!!modalApontar} onClose={() => { setModalApontar(null); setFotoApontar(null) }} title={modalApontar?.finalizando ? `Finalizar Etapa â€” OP #${modalApontar?.opNumero}` : `Apontar ProduÃ§Ã£o â€” OP #${modalApontar?.opNumero}`} centered>
        <Stack gap="md">
          ...
          {modalApontar?.finalizando && !!modalApontar?.quantidadeEtapa && (
            <Group justify="space-between">
              <Text size="xs" c="dimmed">Total jÃ¡ produzido: {(modalApontar.jaProduzido || 0).toLocaleString('pt-BR')} de {modalApontar.quantidadeEtapa.toLocaleString('pt-BR')}</Text>
              <Text size="xs" fw={700} c={((modalApontar.jaProduzido || 0) + formApontar.quantidadeProduzida) >= modalApontar.quantidadeEtapa ? 'green' : 'orange'}>
                {Math.min(100, Math.round((((modalApontar.jaProduzido || 0) + formApontar.quantidadeProduzida) / modalApontar.quantidadeEtapa) * 100))}% concluÃ­do
              </Text>
            </Group>
          )}
          ...
          <Textarea label="ObservaÃ§Ã£o" value={formApontar.observacao} onChange={(e) => setFormApontar({ ...formApontar, observacao: e.currentTarget.value })} />

          {/* Foto da contagem produzida (opcional) â€” evidÃªncia visual anexada
              ao apontamento, ex: foto do contador da mÃ¡quina ou da pilha de
      ...
      {/* Modal: Pausar Etapa */}
      <Modal opened={!!modalPausar} onClose={() => setModalPausar(null)} title={`Pausar Etapa â€” OP #${modalPausar?.opNumero}`} centered>
        <Stack gap="md">
          <Select label="Motivo da Parada" data={[
            { value: 'MANUTENCAO', label: 'ManutenÃ§Ã£o' },
            { value: 'FALTA_MATERIAL', label: 'Falta de Material' },
            { value: 'ACERTO_MAQUINA', label: 'Acerto de MÃ¡quina' },
          ...
          <Textarea label="ObservaÃ§Ã£o" placeholder="Descreva o motivo" value={formPausar.observacao} onChange={(e) => setFormPausar({ ...formPausar, observacao: e.currentTarget.value })} />
      ...
      <Modal opened={!!modalDesmembrar} onClose={() => setModalDesmembrar(null)} title={`Desmembrar â€” OP #${modalDesmembrar?.opNumero}`} centered size="lg">
        <Stack gap="md">
          <Text size="sm" c="dimmed">{modalDesmembrar?.descricao} â€” Total: <strong>{modalDesmembrar?.quantidade?.toLocaleString('pt-BR')} un</strong></Text>
          <Text size="xs" c="orange">A soma das partes deve ser igual Ã  quantidade total ({modalDesmembrar?.quantidade?.toLocaleString('pt-BR')})</Text>

          {formDesmembrar.map((parte, idx) => (
            <Group key={idx} grow>
              <Select
                label={`Parte ${idx + 1} â€” Centro`}
                data={centrosDisponiveis}
                value={parte.centroProducaoId}
                onChange={(v) => { const novo = [...formDesmembrar]; novo[idx].centroProducaoId = v || ''; setFormDesmembrar(novo) }}
                searchable
                placeholder="Selecione a mÃ¡quina"
              />
              <NumberInput
                label="Quantidade"
      ...
      <Modal opened={modalNovoGrupo} onClose={() => setModalNovoGrupo(false)} title="Novo Grupo" centered>
        <Stack gap="md">
          <Select
            ...
            placeholder="Selecione o Tipo de Processo (define a aba onde o grupo aparecerÃ¡)"
            ...
            required
            nothingFoundMessage="Nenhum tipo cadastrado â€” cadastre em Cadastros â†’ Tipo de Processo"
          />
          <TextInput
            label="Nome do Grupo"
            ...
            required
          />
          <Text size="xs" c="dimmed">O grupo serÃ¡ criado vazio. Use o botÃ£o "+" para adicionar OS existentes.</Text>
      ...
      <Modal opened={!!modalAdicionarOS} onClose={fecharModalAdicionarOS} title={`Adicionar OS â€” ${modalAdicionarOS?.centroDescricao}`} centered size="md">
        <Tabs value={tabAdicionarOS} onChange={(v) => setTabAdicionarOS(v as any)}>
          ...
          {/* Aba: vincular OS/OP jÃ¡ cadastrada */}
          <Tabs.Panel value="existente">
            <Stack gap="md">
              ...
              <Textarea
                label="DescriÃ§Ã£o da etapa (opcional)"
          ...
          {/* Aba: OP Avulsa (AV-1, AV-2...) â€” sem nÃºmero de fÃ¡brica, pode ser excluÃ­da a qualquer momento */}
          <Tabs.Panel value="avulsa">
            <Stack gap="md">
              <Text size="xs" c="dimmed">
                Cria uma OP sem nÃºmero de fÃ¡brica (referÃªncia AV-1, AV-2...), Ãºtil para retrabalhos, testes ou
                lanÃ§amentos que nÃ£o vieram de um PDF. Pode ser excluÃ­da a qualquer momento.
              </Text>
              ...
              {modoAvulsa === 'herdar' ? (
                <>
                  ...
                  {opOrigemEncontrada && (
                    <Card withBorder padding="sm" bg="pink.0">
                      ...
                      <Text size="xs" c="dimmed">Produto: {opOrigemEncontrada.produtoNome || opOrigemEncontrada.produto?.nome || opOrigemEncontrada.produtoId || 'NÃ£o vinculado'}</Text>
                      <Text size="xs" c="dimmed">Cliente: {opOrigemEncontrada.clienteNome || opOrigemEncontrada.cliente?.razaoSocial || 'NÃ£o vinculado'}</Text>
                      <Text size="xs" c="dimmed">A avulsa herdarÃ¡ este produto e cliente.</Text>
                    </Card>
                  )}
                ...
                <>
                  <Autocomplete
                    ...
                    placeholder="Buscar produto cadastrado ou digitar descriÃ§Ã£o livre..."
      ...
      <Modal opened={!!modalPostData} onClose={() => { setModalPostData(null); setNovaDataEntrega(null) }} title={`Postergar Entrega â€” OS #${modalPostData?.opNumero}`} centered>
        <Stack gap="md">
          <Text size="sm" c="dimmed">Data atual: <strong>{modalPostData?.dataAtual ? new Date(modalPostData.dataAtual).toLocaleDateString('pt-BR') : 'â€”'}</strong></Text>
          <DatePickerInput
          ...
          >
            Confirmar PostergaÃ§Ã£o
          </Button>
        </Stack>
      </Modal>

      {/* Modal: Retornar etapa concluÃ­da Ã  fila (requer senha admin) */}
      {modalRetornar && (
      <Modal opened onClose={() => { setModalRetornar(null); setFormRetornar({ emailAdmin: '', senhaAdmin: '' }) }} title={`Retornar OS #${modalRetornar.opNumero} Ã  fila`} centered>
        <div onSubmit={(e) => { e.preventDefault(); retornarEtapa() }}>
        <Stack gap="md">
          <Text size="sm" c="dimmed">Esta aÃ§Ã£o retorna a etapa concluÃ­da para status PENDENTE. Requer autorizaÃ§Ã£o de um administrador.</Text>
          <div>
            <Text size="sm" fw={500} mb={4}>UsuÃ¡rio (email do administrador)</Text>
            <input
              type="text"
          ...
          <div>
            <Text size="sm" fw={500} mb={4}>CÃ³digo de autorizaÃ§Ã£o</Text>
            <input
              type="text"
        ...
        <Stack gap="md">
          ...
          <Select
            data={centrosDisponiveis.filter((c: any) => {
              ...
              // Filtrar pela mesma categoria â€” baseado no Tipo de Processo do centro atual
              ...
              if (centroAtual) {
                ...
                // Buscar Tipo de Processo do centro opÃ§Ã£o pelo ID
      ...
      {/* Modal â€” Configurar colunas de impressÃ£o por tipo de processo */}
      <Modal opened={modalColunasImpressao} onClose={() => setModalColunasImpressao(false)} title="Configurar Colunas de ImpressÃ£o" size="lg">
        <Stack gap="md">
          <Text size="sm" c="dimmed">Selecione as colunas que deseja exibir na impressÃ£o para cada tipo de processo.</Text>
      ...
      <style>{`
        ...
        @media print {
          /* Esconder sidebar, header, filtros e botÃµes de aÃ§Ã£o */
          nav, header, aside, .mantine-AppShell-navbar, .mantine-AppShell-header,
          .no-print, [class*="AppShell-navbar"], [class*="AppShell-header"] {
            display: none !important;
          }
          /* Expandir conteÃºdo principal */
          .mantine-AppShell-main, main {
            padding: 0 !important;
            margin: 0 !important;
          }
          /* Esconder grip de drag, filtros e botÃµes de aÃ§Ã£o nas linhas */
          td:first-child, th:first-child,
          td:last-child, th:last-child {
            display: none !important;
          }
          /* Ajustar tamanho da fonte para impressÃ£o */
          table { font-size: 9px !important; }
          th, td { padding: 2px 4px !important; }
          /* ForÃ§ar todas as seÃ§Ãµes abertas */
