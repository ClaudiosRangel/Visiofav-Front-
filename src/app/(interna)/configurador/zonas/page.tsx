...
export default function ZonasPage() {
  ...
  const { data: response, isLoading, refetch } = zonasCrud.useListar({ search: search || undefined })
  const criar = zonasCrud.useCriar()
  const atualizar = zonasCrud.useAtualizar()
  const excluir = zonasCrud.useExcluir()
  ...
  async function onSubmit(data: FormValues) {
    try {
      if (editItem) { await atualizar.mutateAsync({ id: editItem.id, ...data }) }
      else { await criar.mutateAsync(data as any) }
      notifications.show({ title: 'Sucesso', message: editItem ? 'Atualizado' : 'Criado', color: 'green' })
      setModalOpen(false)
  ...
  return (
    <div>
      <Text size="xs" c="dimmed" mb={4}>Início / Configurador / Zonas</Text>
      <Text size="xl" fw={600} mb="lg">Zonas / Bairros</Text>
      <Card pos="relative">
          ...
          <Table.Tbody>
            {items.map((item: any) => (
              <Table.Tr key={item.id}>
                <Table.Td>{item.codigo}</Table.Td><Table.Td>{item.descricao}</Table.Td>
            ...
            {!isLoading && items.length === 0 && <Table.Tr><Table.Td colSpan={4} className="text-center py-8 text-zinc-500">Nenhum registro</Table.Td></Table.Tr>}
          </Table.Tbody>
        </Table>
      </Card>
