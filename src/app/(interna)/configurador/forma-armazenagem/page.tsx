...
import { formasArmazenagemCrud } from '@/data/hooks/useCrudGenerico'

const schema = z.object({ descricao: z.string().min(1, 'Descrição é obrigatória') })
type FormValues = z.infer<typeof schema>

export default function FormaArmazenagemPage() {
  const [modalOpen, setModalOpen] = useState(false)
  const [editItem, setEditItem] = useState<any>(null)
  const [search, setSearch] = useState('')
  const { data: response, isLoading, refetch } = formasArmazenagemCrud.useListar({ search: search || undefined })
  const criar = formasArmazenagemCrud.useCriar()
  const atualizar = formasArmazenagemCrud.useAtualizar()
  const excluir = formasArmazenagemCrud.useExcluir()
  ...
  async function onSubmit(data: FormValues) {
    try { if (editItem) { await atualizar.mutateAsync({ id: editItem.id, ...data }) } else { await criar.mutateAsync(data as any) }; notifications.show({ title: 'Sucesso', message: editItem ? 'Atualizado' : 'Criado', color: 'green' }); setModalOpen(false) }
  ...
  return (
    <div>
      <Text size="xs" c="dimmed" mb={4}>Início / Configurador / Forma Armazenagem</Text>
      <Text size="xl" fw={600} mb="lg">Forma de Armazenagem</Text>
      ...
      <Modal opened={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Editar Forma' : 'Nova Forma'} centered closeOnClickOutside={false}>
