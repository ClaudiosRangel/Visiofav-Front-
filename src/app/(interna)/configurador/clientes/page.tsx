...
export default function ClientesPage() {
  ...
  const inativar = useMutation({
    mutationFn: async (id: string) => { await api.patch(`/clientes/${id}/inativar`) },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['clientes'] }); notifications.show({ title: 'Sucesso', message: 'Cliente inativado', color: 'green' }) },
  ...
  return (
    <div>
      <Text size="xs" c="dimmed" mb={4}>Início / Configurador / Clientes</Text>
      <Text size="xl" fw={600} mb="lg">Clientes</Text>
      ...
      <Card pos="relative">
        ...
        <Group justify="space-between" mb="md">
          ...
          <Group>
            ...
            <Button leftSection={<IconPlus size={16} />} onClick={handleNew}>Novo Cliente</Button>
          </Group>
        </Group>
      ...
      <ClienteModal opened={modalOpen} onClose={() => { setModalOpen(false); setEditItem(null) }} editData={editItem} />
    </div>
  )
}
